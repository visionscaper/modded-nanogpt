...
class Hyperparameters:
    ...
    batch_size : int = no_acc_batch_size # batch size, in sequences, across all devices
    sequence_length : int = sequence_length_MI100 # sequence length, in tokens
    num_iterations : int = num_iterations_MI100 # 1480 # number of iterations to run
    warmup_iters : int = 0
    cooldown_iters : int = 600 # number of iterations of linear warmup/cooldown for triangular or trapezoidal schedule
    weight_decay : float = 0
    # evaluation and logging hyperparams
    val_loss_every : int = 125 # every how many steps to evaluate val loss? 0 for only at the end
    val_tokens : int = sequence_length_MI100 * ddp_world_size * round(reference_val_tokens / (sequence_length_MI100 * ddp_world_size)) # how many tokens of validation data? it's important to keep this fixed for consistent comparisons
